...
void set_all_edge(Graph *G, int height, int width, int label_size, int *I) {
    int i, j, k, tail, head, t_base, h_base, grids_node, grids_edge, source, sink, edge_count, current_edge;
    ...
    // horizontal
    for (i = 1; i <= grids_node - height; i++) {
        t_base = i + (i + i / width) / width;
        h_base = t_base + 1;
        for (j = 0; j < label_size; j++) {
            tail = t_base + j * grids_node;
            for (k = 0; k < label_size; k++) {
                head = h_base + k * grids_node;
                setEdge(G, edge_count, tail, head, 0);
                G->capa[edge_count] = e_cost(j + 1, k + 1);
                edge_count++;
            }
        }
    }
    // vertical
    for (i = 1; i <= grids_node - width; i++) {
        t_base = i;
        h_base = t_base + width;
        for (j = 0; j < label_size; j++) {
            tail = t_base + j * grids_node;
...
void set_single_edge(Graph *G, int height, int width) {
    int i, tail, head, grids_node, source, sink, edge_count;
